...
5. Прикрепляю готовую коллекцию Postman для запросов, для его испольования нужно сделать следующее: ```Postman -> File -> Import``` и выбрать [avito-merch](/pkg/avito-merch.postman_collection.json).
   *(В ней написан скрипт автоматизирующий подстановку полученного токена в header)*
...
- Кэш - написан собственный с использованием **дженериков** и **ttl**, в основе лежит *map* и *mutex* для предовтращения 
_race condition_ и безопасного доступа с во время конкурентности. При каждом получении значения с кэша продлевается время жизни значения.
...
- Написал следующие E2E тесты, для этого создал отдельное пространство в [tests](tests), самостоятельный dockerfile и docker-compose.yaml.
При желании провести тесты можно воспользоваться командами **make**: ```e2e-prepare, e2e-build, e2e-auth, e2e-buy, e2e-send,
e2e-delete```. Команда для создания .env, сборки образов, запуска и проведения теста, соответственно.
